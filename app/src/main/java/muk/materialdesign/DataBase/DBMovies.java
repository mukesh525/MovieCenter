package muk.materialdesign.DataBase;
...
public class DBMovies {
    ...
    public void insertMovies(int table, ArrayList<Movie> listMovies, boolean clearPrevious) {
       ...
       // String sql = "INSERT INTO " + (table == BOX_OFFICE ? MoviesHelper.TABLE_BOX_OFFICE : MoviesHelper.TABLE_UPCOMING) + " VALUES (?,?,?,?,?,?,?,?,?,?);";
        String sql = "INSERT INTO " + (table == BOX_OFFICE ? MoviesHelper.TABLE_BOX_OFFICE :table == UPCOMING? MoviesHelper.TABLE_UPCOMING:MoviesHelper.TABLE_SEARCH) + " VALUES (?,?,?,?,?,?,?,?,?,?);";
    ...
    public ArrayList<Movie> readMovies(int table) {
        ...
        Cursor cursor = mDatabase.query((table == BOX_OFFICE ? MoviesHelper.TABLE_BOX_OFFICE :table == UPCOMING? MoviesHelper.TABLE_UPCOMING:MoviesHelper.TABLE_SEARCH), columns, null, null, null, null, null);
    ...
    public void deleteMovies(int table) {
        mDatabase.delete(table == BOX_OFFICE ? MoviesHelper.TABLE_BOX_OFFICE :table == UPCOMING ? MoviesHelper.TABLE_UPCOMING: MoviesHelper.TABLE_SEARCH, null, null);
    }

    private static class MoviesHelper extends SQLiteOpenHelper {
        public static final String TABLE_UPCOMING = " movies_upcoming";
        public static final String TABLE_SEARCH = " movies_search";
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            try {
                ...
                db.execSQL(" DROP TABLE " + TABLE_SEARCH+ " IF EXISTS;");
                onCreate(db);
            } catch (SQLiteException exception) {
                L.t(mContext, exception + "");
            }
